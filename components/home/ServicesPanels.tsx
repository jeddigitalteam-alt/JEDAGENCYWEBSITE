 ...
 *
 * Every card sits at the size the old hover-expanding panel only reached when
 * you hovered it: that panel gave the hovered card `flex: 2.2` against a total
 * of 5.32, which on a 1440px viewport worked out at 562px — 39vw. So 39vw is
 * the width here, and it tracks that size at every viewport rather than at one.
 *
...
export function ServicesPanels() {
  ...
  return (
    <section ref={sectionRef} className="px-5 py-24 md:px-8 md:py-32">
      ...
      {/* Breaks the page gutter so the strip runs edge to edge. `overflow-hidden`
          is what keeps the second copy off the page's own scroll width. Under
          reduced motion the same box becomes an ordinary scroller. */}
      <div className="-mx-5 overflow-hidden md:-mx-8 motion-reduce:overflow-x-auto motion-reduce:overscroll-x-contain motion-reduce:px-5 motion-reduce:[scrollbar-width:none] md:motion-reduce:px-8 [&::-webkit-scrollbar]:hidden">
        <ul
            ...
            SHOWN.map((service, i) => {
              ...
              return (
                <li
                  ...
                  className="group relative mr-1 h-[20rem] w-[86vw] shrink-0 overflow-hidden rounded-xl border border-rule bg-ink-raised md:h-[32rem] md:w-[46vw] lg:w-[39vw]"
                >
