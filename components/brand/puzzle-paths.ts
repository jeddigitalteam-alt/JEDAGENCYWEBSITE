 ...
 * between contexts.
 *
 * The two subpaths SHARE their seam boundary: piece A's edge runs
 * (343.5,343.6) → (462.0,462.1) → (657.1,655.9) and piece B's runs
 * (342.3,344.1) → (655.3,656.4) → (553.8,554.9). Their locked resting state is
 * the path data exactly as written — knob seated in socket, edge against edge.
 * The seam therefore renders as a single line down the middle of the mark.
 *
 * DO NOT scale, nudge or transform either piece independently in a resting
 * state. Any gap along the seam means it is wrong. The only legal per-piece
 * transform is the loader's lock animation, which starts the pieces apart
 * along SEAM_NORMAL and always ends on `transform: none`.
 */
...
/**
 * The seam runs down-right at ~45°: (343,344) → (657,656), direction ≈ (1,1)/√2.
 * The normal is therefore (1,-1)/√2 — piece A separates up-right, piece B
 * down-left. Used only by the loader's lock animation.
 */
